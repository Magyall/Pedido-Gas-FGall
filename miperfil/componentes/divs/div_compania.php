...
for($i=0;$i<count($datos_consulta);$i++){
	$datos=$datos_consulta[$i]['Id']."||".
			...
			$datos_consulta[$i]['His']."||".
			$datos_consulta[$i]['Mis']."||".
			$datos_consulta[$i]['Vis']."||".
			$datos_consulta[$i]['Log']."||".
...
?>
		<tr>
			...
			<td>
<?php
				if($datos_consulta[$i]['Log'] == 'Sinfoto'){
?>
				<button class="btn btn-outline-secondary" data-toggle="modal" data-target="#modallogo" onclick="cargar_datos('<?php echo $datos ?>')">
					<i class="nav-icon fa fa-image"></i>
				</button>
<?php
				}else{
?>
				<img src="../complementos/images/<?php echo $datos_consulta[$i]['Log'] ?>" style="height: 30px" data-toggle="modal" data-target="#modallogo" onclick="cargar_datos('<?php echo $datos ?>')">
<?php
				}
?>
			</td>
...
?>
				<img src="../complementos/images/<?php echo $datos_consulta[$i]['Img'] ?>" style="height: 30px" data-toggle="modal" data-target="#modalimagen" onclick="cargar_datos('<?php echo $datos ?>')">
<?php
				}
?>
			</td>
